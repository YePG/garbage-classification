...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # Run the training for as many cycles as reque
    # sted on the command line.
   ## //训练预先设定的次数，脚本默认500次
    for i in range(FLAGS.how_many_training_steps):
      ##//获取数据集的一部分（batch）进行输入，判断是否需要调整图片
      if do_distort_images:
        (train_bottlenecks,
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        tf.summary.scalar('Cross_entropy_value',cross_entropy_value)
